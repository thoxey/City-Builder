...
func test_resolved_request_copies_path_and_preserves_identity() -> void:
	...
	assert_gte(jid, 0)
	path.clear()
	var row: Dictionary = manager.get_civilian_snapshot()["journeys"][0]
...
func test_pool_full_rejects_without_re_resolving_or_fallback() -> void:
	var manager := _manager(0)
	var path: Array[Vector3i] = [Vector3i.ZERO,Vector3i(1,0,0)]
	assert_eq(manager.request_resolved_journey(1,path.front(),path.back(),path,2,"p"), -1)
